    ...
    override func layoutSubviews() {
        ...
        else
        {
            ...
            LGF_slideView!.subviews.enumerated().forEach { (index,view) in
                view.frame = CGRect(x:LGF_ButtonMargin * CGFloat(temp - 1 - index), y: 0, width: LGF_ButtonMargin * CGFloat(index + 1) + bounceDistance, height: frame.height)
            }
...
//            LGF_containerView.contentSize = CGSize(width: LGF_SCREEN_WIDTH + LGF_ButtonMargin * CGFloat(temp), height: bounds.height)
        }
